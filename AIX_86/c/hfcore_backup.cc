...
void aio_handler(int signal, siginfo_t *info, void *uap)
{
	...
	struct aiocb* io = req->ioo;
	int pid = req->pid;
	int cbNumber = req->id;
	endTime[pid][cbNumber] = core_time-base_time;
	glbsize[pid][cbNumber] = io->aio_nbytes;
	if (aio_error(io) == (EAGAIN | EINVAL | EINTR | EIO) ) printf("ERRNO=%d STR=%s\n", aio_error(io), strerror(aio_error(io)));
	if (aio_return(io) != io->aio_nbytes)  
		printf("lio_list did read/write %d bytes, expecting %d bytes at #%d\n", aio_return(io), io->aio_nbytes, cbNumber);
	io_complete[0]=io_complete[0]+1;
	//printf("Complete Req No#%d tot#: %d time %f\n", cbNumber, io_complete[pid], float(endTime[pid][cbNumber])/1000000000);
    ...
    **********************************************************************/
    ...
    int prepareIOs()
    {
        ...
        for(i = 0; i < WT ; i++)
        {
			...
			glbsize[i] = new unsigned long [perThreadBundles];
		...
		for (i = 0; i < totalBundles; i++){
			//printf("IO request#%d	size: %d  offset %d\n", i, params[0].ioq[i].iocb_bunch[0]->aio_nbytes, params[0].ioq[i].iocb_bunch[0]->aio_offset);
    ...
    *************************************************************************/
    ...
    static void* executeHarvester(void* threadid)
    {
		...
		*/
        ...
        pid = getpid();
		int retbind=0;
		cpu_t bindparameter = 2;
		retbind = bindprocessor(BINDTHREAD, pthread_self(), bindparameter);
		printf("Selfthread %d %d\n", pthread_self(), pid);
		if (retbind == -1) 
		{
			printf("Harvester    Failed to set Thread Affinity  %d\n", errno);
		...
		}else if ( retbind == 0) printf("Harvester thread running on Core %d\n", bindparameter);
	   ...
	   for (unsigned cnt = 0; cnt < WT; cnt++)
		...
		action.sa_flags =SA_NODEFER | SA_SIGINFO; //SA_NODEFER
		sigemptyset(&action.sa_mask);
		sigaction(SIG_AIO, &action, NULL);
			
/*		
		doneflag = 0;
		while (doneflag != active_WT){
			 ...
			 for (unsigned i = 0; i < WT; i++){
				...
				//if (sigaction(SIG_AIO, &action, NULL) == 0) printf("________________________________________ \n");
		*/
       ...
       while (1)
        {
			...
			/*
			numevents = aio_nwait(queuelen, 0, list);
			printf("evenumver size %d \n", numevents);
                    if(numevents > 0)
                    {
						io_complete[0] = io_complete[0]+numevents;
                        for(int j = 0; j < numevents; j++)
                        {
                            struct aiocb * ioevent = list[j];
							printf("io size %d \n", ioevent->aio_nbytes);
                        }
					}
			*/
			//printf("IO_complete %d \n", io_complete[0]);
			//printf("AIO operation %d completed returning %d \n", cbNumber,	aio_return(ioo[cbNumber]));
            for(unsigned i = 0; i < active_WT; i++)
            {
				
                if (io_complete[i] >= io_count[i]) break;
                    //doneflag++;
    ...
    static void* executeTimer(void* threadid)
    {
		...
		*/
        ...
        while ((cycle_count - warm_start) / (max_speed) < 1000000)
        {
            cycle_count = rdtscp();
			//printf("Timer warmup complete %llu\n", cycle_count);
        }
    ...
    static void* executeWorker(void* param_p)
    {
		...
		*/
		...
		cpu_t bindparameter = tid + first_worker;
/*
		pid_t  pid;
        pid = getpid();
		int retbind=0;		
		retbind = bindprocessor(BINDTHREAD, pid, bindparameter);
		printf("Selfthread %d %d\n", pthread_self(), pid);
		if (retbind == -1) 
		{
			printf("Worker    Failed to set Thread Affinity  %d\n", errno);
			if (errno == EINVAL) printf("The What parameter is invalid\n");
			if (errno == ESRCH) printf("The specified process or thread does not exist\n");
			if (errno == EPERM) printf("The caller does not have root user authority\n");
		}else if ( retbind == 0) printf("Worker thread running on Core %d\n", bindparameter);
		*/
		...
		pid[0]=4;
		rsid_t rsid1, rsid2;
		rsid1.at_tid = pthread_self();
		rsid2.at_rset = pid;
		int retbind=0;
		retbind = ra_attach(R_THREAD, rsid1, R_RSET, rsid2, P_DEFAULT);
		if (retbind == -1){
			printf("Worker    Failed to set Thread Affinity  %d\n", errno);
			if (errno == EINVAL) printf("The What parameter is invalid\n");
			if (errno == ENODEV) printf("The specified process or thread does not exist\n");
			if (errno == ENOTSUP) printf("The caller does not have root user authority\n");
			if (errno == ESRCH) printf("ESRCH\n");
		...
		*/
        ...
        for(int i = 0; ioq[i].iocb_bunch != NULL ; i++)
        {
			...
			sig[tid].sigev_notify = SIGEV_SIGNAL;
			sig[tid].sigev_signo = SIG_AIO;
			iooq* req = new iooq(ioq[i].iocb_bunch[0], i, tid);
			//assert(req);
			sig[tid].sigev_value.sival_ptr = req;
			...
			//printf("IO request#%d	size: %d  offset %d\n", i, io_count[0], ioq[i].iocb_bunch[0]->aio_offset);
			beginTime[tid][i]=core_time - base_time; 
			if( lio_listio(LIO_NOWAIT, ioq[i].iocb_bunch, 1, sig) !=-1){ //LIO_NOWAIT_AIOWAIT	LIO_NOWAIT
			//if(aio_read(ioq[i].iocb_bunch[0]) !=-1){
					issueTime[tid][i]=core_time - base_time;
					//printf("Submit IO request#%d	size: %d  time %f\n", i, ioq[i].iocb_bunch[0]->aio_nbytes, float(issueTime[tid][i])/1000000000);
...
private:
    ...
    {
        vector<struct aiocb> currentBundle;
		vector<struct AIORequest*> currentReq;
        DRecordData* rec;
        ...
        while((rec = dump->next()))
        {
            ...
            if(op < IOLogTrace::max_optypes)
            {
                ...
                if(cmp == 0)  // <0 means skip this IO, out of range
                {
                    ...
                    {
                        struct aiocb lastIocb = currentBundle.back();
						struct AIORequest* lastReqP = currentReq.back();
                        ...
                        else
                        {
                            ...
                            struct aiocb firstIocb = currentBundle.front();
							struct AIORequest* firstReqP = currentReq.front();
                            newBundleP->startTimeNano = firstReqP->ts ;
                            newBundleP->startLoad = startLoad ;

                            for(unsigned i = 0; i < currentBundle.size() ; i++)
                            {
                                newBundleP->iocb_bunch[i] = new struct aiocb;
        ...
        if(currentBundle.size())
        {
            ...
            struct aiocb firstIocb = currentBundle.front();
            struct AIORequest * firstAIOreq = currentReq.front();
            lastBundleP->startTimeNano = firstAIOreq->ts ;

            for(unsigned i = 0; i < currentBundle.size() ; i++)
            {
                lastBundleP->iocb_bunch[i] = new struct aiocb;
