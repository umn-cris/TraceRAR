 ...
 **    Authors:  Sai Susarla, Weiping He, Jerry Fredin, Ibra Fall, Clark Anderson, Bingzhe Li
 **
 ******************************************************************************
 **
...
#endif /* 0 */
		//printf("cfg info: LUN# %d, lba_start %u, range %llu\n", lun, (*rec)["start_offset"].i, (*rec)["range_nbytes"].i);
    }
    ...
    unsigned long long& iotime) /* out in nanosecond */
{
    ...
    if ((usecs < cfg.start_usecs) || (usecs >= (cfg.start_usecs + cfg.num_usecs))){
		printf("-----------filter #1 num: %llu \n", usecs); //By Bingzhe
        return -1;
	}
    
	//printf("------------filter #2 lun: %d lba %lld size %d %d  %lld %lld \n", lun, lba, sizeof(ULONG64), sizeof(uint64_t), cfg.lba_start, cfg.nblks); //By Bingzhe
