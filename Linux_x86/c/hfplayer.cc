...
unsigned int NRR = 0; // number of requests regenerated, default is 0
unsigned int RAR = 0; // number of requests regenerated, default is 0
int doCapture = 0;
int shortIATnsec = 0; //turns of bundling by default
int stopOnError = 0;
int stopOnOverload = 0;
int max_inflight = 8192;
...
{
    fprintf(stderr,
            "Usage: %s -v -c <cpu_clock_speed> -so -se -wlc -cfg <config_file> -nt <threads> -rar <turn on/off replayer, analyzer and regenerator> -regenerator <number of I/O requests regenerated> -q <max outstanding IOs> <trace_file>.csv\n\n\
    -c <cpu clock speed>\n\
    ...
    -reg <number of I/O requests regenerated>\n\
                ...
                if (strcmp(argv[i], "-cfg") == 0)
                {
                    ...
                    if (strcmp(argv[i], "-q") == 0)
                    {
                        ...
                        if (strcmp(argv[i], "-nt") == 0)
                        {
							...
							if(strcmp(argv[i], "-rar") == 0)
							{
								if (++ i >= argc)
								{
									fprintf(stderr, "supply number of threads to -regenerator\n");
									usage();
								}
								RAR = atoi(argv[i++]);
								if(RAR != 1 & RAR != 2 & RAR != 3 & RAR != 4 & RAR != 5){
									usage();
								}
								if( RAR == 3 | RAR == 4 | RAR ==5)
								{
									if (strcmp(argv[i], "-reg") == 0)
									{
										...
										fprintf(stderr, "ERROR: Need -reg <# of ops> following with -rar with 3 or 4\n");
										exit(1);
									}
									NRR = atoi(argv[i++]);
								}
