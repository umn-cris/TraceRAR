...
#define Mshift 10            // shift for max and min value
...
public:
	...
	probability prob;
	long totio;
	//******* Average value of different metrics for separate read and write
	unsigned long long ave_size_r;
	unsigned long long ave_size_w;
	unsigned long long ave_time_r;
	unsigned long long ave_time_w;
	unsigned long long ave_offset_r;
	unsigned long long ave_offset_w;
	
	//*****regression model******//
	double a_size[granularity];
	double b_size[granularity];
	double a_offset[granularity];
	double b_offset[granularity];
	double a_time[granularity];
	double b_time[granularity];
	double a_RatioRW;
	double b_RatioRW;
	double a_seq;
	double b_seq;
...
unsigned long long totreq = 116384;// 520000;//13000000;
float executiontime = 0;
int fine_granularity = 1; // fined granularity analysis On/Off, 1 is on
...
public:
    ...
    **********************************************************************/
    int run()
    {
		...
		issueTime = new unsigned long long[totreq];
		wallTime= new unsigned long long[totreq];
		endTime= new unsigned long long[totreq];
		...
		if( RAR == 2 | RAR == 3 | RAR == 4 | RAR == 5){
			...
			printf("Total: %d, The read#: %d, Write#: %d	Ratio of R: %f\n",totnumreq, analysis.RatioRW.read, analysis.RatioRW.write, float(analysis.RatioRW.read)/float(totnumreq)); //analysis.RatioRW.read+analysis.RatioRW.write
			
			printf("Average Size: Write(KB) :%f, Read#: %f\n", float(analysis.ave_size_w)*float(analysis.RatioRW.write)/float(analysis.RatioRW.read+analysis.RatioRW.write)/float(totnumreq), float(analysis.ave_size_r)*float(analysis.RatioRW.read)/float(analysis.RatioRW.read+analysis.RatioRW.write)/float(totnumreq));
			printf("Average inter-arrive time: Write(us) :%f, Read#: %f\n", float(analysis.ave_time_w)*float(analysis.RatioRW.write)/float(totnumreq)/float(totnumreq)/1000, float(analysis.ave_time_r)*float(analysis.RatioRW.read)/float(totnumreq)/float(totnumreq)/1000);
			printf("Average offset(MB): Write :%f, Read#: %f\n", float(analysis.ave_offset_w)*float(analysis.RatioRW.write)/float(totnumreq)/float(totnumreq)/1024/1024, float(analysis.ave_offset_r)*float(analysis.RatioRW.read)/float(totnumreq)/float(totnumreq))/1024/1024;
			if(fine_granularity == 1){
				FILE * pFile1;
				pFile1 = fopen ("../result/size_fine.csv","w");
				fprintf(pFile1, "#Size,Prob,a,b\n");
				for (int i = 0; i < analysis.prob_size.size(); i++) 
					fprintf(pFile1, "%llu,%f\n", analysis.number_size[i] / 1024, analysis.cdf_size[i]);
				fclose (pFile1);
				
				FILE * pFile2;
				pFile2 = fopen ("../result/offset_fine.csv","w");
				fprintf(pFile2, "#Locality,	Prob,	a,	b\n");
				for (int i = 0; i < analysis.prob_offset.size(); i++) 
					fprintf(pFile2, "%llu,%f\n", analysis.number_offset[i]/1024/1024, analysis.cdf_offset[i]);
				fclose (pFile2);
				
				FILE * pFile3;
				pFile3 = fopen ("../result/time_fine.csv","w");
				fprintf(pFile3, "#Time,	Prob,	a,	b\n");
				for (int i = 0; i < analysis.prob_time.size(); i++) 
					fprintf(pFile3, "%llu,%f\n", analysis.number_time[i]/1000, analysis.cdf_time[i]);
				fclose (pFile3);
			}else{				
				FILE * pFile1;
				...
				for (int i = 0; i < granularity; i++) 
					fprintf(pFile1, "%llu,%f,%E,%E\n", IntervalSize[i] / 1024, analysis.prob.P_size[i], analysis.a_size[i], analysis.b_size[i]);
				fclose (pFile1);
				...
				fprintf(pFile2, "#Locality,Prob,a,b\n");
				for (int i = 0; i < granularity; i++) 
					fprintf(pFile2, "%llu,%f,%E,%E\n", IntervalOffset[i]/1024/1024, analysis.prob.P_offset[i], analysis.a_offset[i], analysis.b_offset[i]);
				fclose (pFile2);
				...
				fprintf(pFile3, "#Time,Prob,a,b\n");
				for (int i = 0; i < granularity; i++) 
					fprintf(pFile3, "%llu,%f,%E,%E\n", IntervalTime[i]/1000, analysis.prob.P_time[i], analysis.a_time[i], analysis.b_time[i]);
				fclose (pFile3);
	...
	int doAnalysis(TraceAnalysis* analysis)
    {
		...
		int base = 1000000;
		int reg_number=0;
		int totalreg = (totnumreq-base)/regpoint;
		double reg_size[regpoint][granularity];
		double reg_offset[regpoint][granularity];
		double reg_time[regpoint][granularity];
		
		double reg_RatioRW[regpoint];
		double reg_seq[regpoint];
		...
		while(count <=totreq &  bool(rec = dump->next()) ){
            ...
            if(op < IOLogTrace::max_optypes)
            {
                ...
                if(cmp == 0)  // <0 means skip this IO, out of range
                {
					...
					if (it == analysis->number_lun.end()) {
						analysis->number_lun.push_back(lun);
					}
					//************* arrival rate ********************
					unsigned long long time = 0;
					newtime = iotime;
					if (count != 1){
						time = newtime - pretime;  //unit is micron second(us)
					...
					//*********** Read Write Ratio **********//
					//printf("#%d %llu %llu %llu\n", count, time, size, offset);
                    if(op == 1)  // write op
                    {
						analysis->RatioRW.write++;
						analysis->ave_time_w = analysis->ave_time_w + time;
						analysis->ave_size_w = analysis->ave_size_w + size;
						analysis->ave_offset_w = analysis->ave_offset_w + offset;
                    }
                    else  // read op
                    {
						analysis->RatioRW.read++; 
						analysis->ave_time_r = analysis->ave_time_r + time;
						analysis->ave_size_r = analysis->ave_size_r + size;
						analysis->ave_offset_r = analysis->ave_offset_r + offset;
                    }
					//printf("#%d %llu %llu\n", count,  analysis->ave_size_w/512, analysis->ave_offset_w/512);
					pretime = newtime;
				}
			}
					
			
			// reqression model for recording probabilities
			if(count >= base + totalreg * (reg_number+1)){
				for (int k = 0; k < granularity ; k++){
					reg_size[reg_number][k] = double(analysis->size[k])/count;
					reg_offset[reg_number][k] = double(analysis->offset[k])/count;
					reg_time[reg_number][k] = double(analysis->time[k])/count;
					//printf("#%d	%f	%f\n",reg_number, reg_size[reg_number][k], reg_offset[reg_number][k]);
				}
				reg_RatioRW[reg_number]=double(analysis->RatioRW.read)/count;
				reg_seq[reg_number]=double(analysis->seq)/count;
				
				//printf("%d	%f\n",count, reg_size[reg_number][0]);
				//printf("%d,%f,%f,%f,%f,%f\n",count, reg_size[reg_number][0], reg_offset[reg_number][0], reg_time[reg_number][0],reg_RatioRW[reg_number], reg_seq[reg_number]);
				reg_number++;
			}
		...
		/*
		for (unsigned i = 1; i < granularity ; i++){
				printf("#%d %d %d\n", analysis->offset[i], analysis->size[i], analysis->time[i]);
			}
		*/
		/************* regression model ***************/
		
		double Sx = 0;
		double sizebar[granularity];
		double Sxy_size[granularity];
		double b_size[granularity];
		double a_size[granularity];
		
		double offsetbar[granularity];
		double Sxy_offset[granularity];
		double b_offset[granularity];
		double a_offset[granularity];
		
		double timebar[granularity];
		double Sxy_time[granularity];
		double b_time[granularity];
		double a_time[granularity];
		...
		double RatioRWbar=0;
		double Sxy_RatioRW=0;
		double b_RatioRW;
		double a_RatioRW;

		for (int m = 0; m < regpoint  ; m++){
				double temp = abs(int(totalreg*(m+1) - (totalreg+totnumreq)/2));
				Sx = Sx + pow(temp, 2.0);
				//printf("Sx: %f, %f\n", Sx, temp);
		}
		...
		for (int k = 0; k < granularity ; k++){
			...
			timebar[k] = 0;
			for (int m = 0; m < regpoint  ; m++){
				sizebar[k] = sizebar[k] + reg_size[m][k];
				offsetbar[k] = offsetbar[k] + reg_offset[m][k];
				timebar[k] = timebar[k] + reg_time[m][k];
			}
			sizebar[k] = sizebar[k]/regpoint;
			offsetbar[k] = offsetbar[k]/regpoint;
			timebar[k] = timebar[k]/regpoint;
			//printf("#	%f", sizebar[k]);
		}
		for (int m = 0; m < regpoint  ; m++){
				seqbar = seqbar + reg_seq[m];
				RatioRWbar = RatioRWbar + reg_RatioRW[m];
		}
		
		for (int k = 0; k < granularity ; k++){
			Sxy_size[k] = 0;
			for (int m = 0; m < regpoint  ; m++){
				double temp = int(totalreg*(m+1) - (totalreg+totnumreq)/2);
				Sxy_size[k] = Sxy_size[k] + temp*(reg_size[m][k] - sizebar[k]);
				Sxy_offset[k] = Sxy_offset[k] + temp*(reg_offset[m][k] - offsetbar[k]);
				Sxy_time[k] = Sxy_time[k] + temp*(reg_time[m][k] - timebar[k]);
				
				//if (k==1) printf("Sx: %f, Sxy: %f %e \n", Sx, temp, temp*reg_size[m][k]- sizebar[k]);
			}
		}
		for (int m = 0; m < regpoint  ; m++){
				Sxy_seq = Sxy_seq + int(totalreg*(m+1) - (totalreg+totnumreq)/2) * (reg_seq[m] - seqbar);
				Sxy_RatioRW = Sxy_RatioRW + int(totalreg*(m+1) - (totalreg+totnumreq)/2) * (reg_RatioRW[m] - RatioRWbar);
		}
	
		for (int k = 0; k < granularity ; k++){
			b_size[k] = Sxy_size[k]/Sx;
			a_size[k] = sizebar[k] - b_size[k] * int(totalreg+totnumreq/2);
			
			b_offset[k] = Sxy_offset[k]/Sx;
			a_offset[k] = offsetbar[k] - b_offset[k] * int(totalreg+totnumreq/2);
			
			b_time[k] = Sxy_time[k]/Sx;
			a_time[k] = timebar[k] - b_time[k] * int(totalreg+totnumreq/2);
			
			analysis->b_size[k] = Sxy_size[k]/Sx;
			analysis->a_size[k] = sizebar[k] - analysis->b_size[k] * int(totalreg+totnumreq/2);
			
			analysis->b_offset[k] = Sxy_offset[k]/Sx;
			analysis->a_offset[k] = offsetbar[k] - analysis->b_offset[k] * int(totalreg+totnumreq/2);
			
			analysis->b_time[k] = Sxy_time[k]/Sx;
			analysis->a_time[k] = timebar[k] - analysis->b_time[k] * int(totalreg+totnumreq/2);
			
		}
		b_seq = Sxy_seq/Sx;
		a_seq = seqbar - b_seq * int(totalreg+totnumreq/2);
		
		analysis->b_seq = Sxy_seq/Sx;
		analysis->a_seq = seqbar - analysis->b_seq * int(totalreg+totnumreq/2);
		
		analysis->b_RatioRW = Sxy_RatioRW/Sx;
		analysis->a_RatioRW = RatioRWbar - b_RatioRW * int(totalreg+totnumreq/2);
		
		b_RatioRW = Sxy_RatioRW/Sx;
		a_RatioRW = RatioRWbar - b_RatioRW * int(totalreg+totnumreq/2);
		//for (int k = 0; k < granularity ; k++){
		printf("//******* regresion model: Y=a+bX *********//\n");
		printf("size: a: %e	b: %e\n",a_size[0], b_size[0]);
		printf("offset: a: %e	b: %e\n",a_offset[0], b_offset[0]);
		printf("time: a: %e	b: %e\n",a_time[0], b_time[0]);
		printf("seq: a: %e	b: %e\n",a_seq, b_seq);
		printf("RatioRW: a: %e	b: %e\n",a_RatioRW, b_RatioRW);
		//}
	...
	int regenerator(TraceAnalysis* analysis)
	{
		...
		unsigned long long offset, offset1, size;
		unsigned op;
		offset = 0;
		offset1 = 0;
		dump->restart();
		...
		{	
			...
			while(count <=totreq &  bool(rec = dump->next()) ){
				...
				int lun;
					...
					if(cmp == 0)  // <0 means skip this IO, out of range
					{
						...
						op = (*rec)["op"].i;
						fprintf (pFile, "0,%llu,0,0,0,0,0,6,%llu,0,%llu,%llu,0,0,0,0\n", iotime, op, offset, size/DISK_BLOCK_SIZE);
					}
					if(number <= 0) break;
			}
				
			if(count > totreq | !bool(rec = dump->next()))	{
			...
			for (int i = 0; i < analysis->cdf_offset.size(); i++){
				if(random1 <= analysis->cdf_offset[i]) {
					offset = analysis->number_offset[i] / DISK_BLOCK_SIZE;
					break;
				}
			}
			...
			random4 = float(rand() % 1000000)/1000000;
			if(random4 <= PWR) {
				op = 1;
			}else op = 0;
			...
			if(number == 0) printf ("Last timestamp is %llu  %f\n", offset, analysis->P_WR);
		//fprintf (pFile, "0	%llu	0	0	0	0	0	6	%u	0	%llu	%llu	0	0	0	0\n", time, op, offset, size);
		}
		fclose (pFile);
    ...
    *************************************************************************/
	  ...
	  static void* executeHarvester(void* threadid)
    {
       ...
       while (1)
            ...
            for(unsigned i = 0; i < 1; i++)
            {
                ...
                {	
                    ...
                    for(int j = 0; j < numevents; j++)
                    {
						...
						endTime[req->requestID]=core_time - base_time;
                        ...
                        if(event.res != req->bufsize)
                        {
                            ...
                            {
                                unkerr++;
                                printf("Unknown error returned on aio completion: %d %d  %d\n",req->requestID, event.res, req->bufsize);
                            }
                        }
						delete (event.data);
						//delete event;
		...
		finishtime = (float)(core_time - base_time) / (long double) 1000000000;
        for (j = 0; j < WT ; j++)
        {
		...
		//fflush(1);
		for (int i = 0; i < totalIOs; i++)
			executiontime = (float)(endTime[i]-issueTime[i])/1000000000 + executiontime;
		
		
        printf("Completed %d IO Bundles in %Lf seconds, %Lf Bundle/S\n", totalIOs, (long double)(core_time - base_time)/ 1000000000, (long double) totalIOs / ((long double)(core_time - base_time) / (long double)1000000000) );
        printf("Completed %d IO in %f seconds, %LF IOPS\n", totalIOs, finishtime, (long double)totalIOs / ((long double)(core_time - base_time) / (long double)1000000000 ) );
		printf("Total time #: %f \n",executiontime);
		printf("Ave excution time: %f seconds\n", executiontime/totnumreq);
		
		
		//FILE * pFile2;
		//pFile2 = fopen ("timestamp","w");
		double issue_error = 0;
		for (int i = 0; i < totalIOs; i++){ //150828  //566407 //32768{
				//fprintf(pFile2, "%d	%f	%f	%f\n",i, (float)wallTime[i]/1000000000, (float)issueTime[i]/1000000000, (float)(issueTime[i]-wallTime[i])/1000000000);
			issue_error = (float)(issueTime[i]-wallTime[i])/1000000000 + issue_error;
		}
		//fclose (pFile2);
		printf("Issue error time: %f us\n", issue_error/totnumreq*1000000);
		printf("Done\n");
    ...
    static void* executeWorker(void* param_p)
    {
        ...
        while(flag){
			...
			//if(*ioq[i].iocb_bunch== NULL){
				...
				//printf("IO request#%llu	size: %d  offset %llu wtime %f itime %f iov %d Complete: %d \n", count, (*it)->u.c.nbytes, (*it)->u.c.offset, float(core_time - base_time)/1000000000, float(wallTime[count])/1000000000, iov.size(), io_complete[0]);
				issueTime[count] =  core_time - base_time;
				//printf("%f,%f,%f \n",(float)wallTime[count]/1000000000, (float)issueTime[count]/1000000000, (float)(issueTime[count]-wallTime[count])/1000000000);
